					...
					ALL_SCRIPTS
					{
						...
						else
						{
							...
							*/
							...
							// I don't know, where luaG_inter_copy_from comes from, replace it with luaG_inter_copy for now - UKnowMe?
							// luaG_inter_copy_from( donor, SCRIPT, n, origin );
							luaG_inter_copy(donor, SCRIPT, n);
						}
					}
					for ( int i = 0; i < n; ++i )
...
static bool checkArguments( lua_State *L, const char *format, const char **desc = NULL )
{
	lua_checkstack( L, strlen( format ) );
	for ( int i = 0; *format; ++i )
	{
		int expectation = LUA_TNONE;
		bool expected = true;
		switch ( format[i] )
		{
			case 'n':
				expected = lua_isnumber(L, i + 1);
				expectation = LUA_TNUMBER; break;
			case 's':
				expected = lua_isstring(L, i + 1);
				expectation = LUA_TSTRING; break;
			case 'b':
				expected = lua_isboolean(L, i + 1);
				expectation = LUA_TBOOLEAN; break;
			case 'f':
				expected = lua_isfunction(L, i + 1);
				expectation = LUA_TFUNCTION; break;
			case 't':
				expected = lua_istable(L, i + 1);
				expectation = LUA_TTABLE; break;
		}
		if ( !expected)
		{
			if ( desc )
			{
				...
				sprintf(
						message,
						"Expected %s for argument '%s', got: %s",
						lua_typename( L, expectation ),
						desc[i],
...
{
	lua_pushinteger( L, ( gamelimit - gamemillis + 60000 - 1 ) / 60000 );
	return 1;
}
...
LUA_FUNCTION (settimeleft)
{
	...
	intermission_check_shear = 60 * 1000 - gamemillis % (60 * 1000) + framemillis;
...
LUA_FUNCTION (ban)
{
	...
	int ban_type = BAN_AUTO;
	if ( lua_isnumber( L, 3 ) ) ban_type = (int) lua_tonumber( L, 3 );
	ban ban = {
		.address = clients[target_cn]->peer->address,
		.millis = servmillis + ban_time,
		.type = ban_type
	};
	bans.add( ban );
...
LUA_FUNCTION (getlastsay)
{
	lua_checkstack( L, 1 );
	if ( !lua_isnumber( L, 1 ) ) return 0;
	int player_cn = (int) lua_tonumber( L, 1 );
	if ( !valid_client( player_cn ) ) return 0;
	lua_pushinteger( L, clients[player_cn]->lastsay );
	return 1;
}

LUA_FUNCTION (getlastsaytext)
{
	lua_checkstack( L, 1 );
	if ( !lua_isnumber( L, 1 ) ) return 0;
	int player_cn = (int) lua_tonumber( L, 1 );
	if ( !valid_client( player_cn ) ) return 0;
	lua_pushstring( L, clients[player_cn]->lastsaytext );
	return 1;
}

LUA_FUNCTION (authclient)
{
	lua_checkstack( L, 2 );
	if ( !lua_isnumber( L, 1 ) ) return 0;
	...
	int target_cn = (int) lua_tonumber( L, 1 );
	if ( !valid_client( target_cn ) ) return 0;
	int authed = (int) lua_toboolean( L, 2 );
	clients[target_cn]->isauthed = authed;
	return 0;
}

LUA_FUNCTION (isauthed)
{
	lua_checkstack( L, 1 );
	if ( !lua_isnumber( L, 1 ) ) return 0;
	int player_cn = (int) lua_tonumber( L, 1 );
